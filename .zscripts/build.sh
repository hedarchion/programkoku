...
# Next.js 项目路径（优先使用环境变量，其次使用脚本上级目录）
DEFAULT_PROJECT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
NEXTJS_PROJECT_DIR="${NEXTJS_PROJECT_DIR:-$DEFAULT_PROJECT_DIR}"
...
# 复制 Next.js 静态导出产物（output: export）
if [ -d "out" ]; then
    echo "  - 复制 out (static export)"
    mkdir -p "$BUILD_DIR/next-service-dist"
    cp -r out "$BUILD_DIR/next-service-dist/"
else
    echo "❌ 未找到 out 目录，请检查 next.config.ts 是否为 output: export"
    exit 1
fi
